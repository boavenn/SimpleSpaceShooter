...
ParticleExplosion::ParticleExplosion(sf::Vector2f pos, float max_life_time, unsigned size) : Effect(pos, max_life_time)
{
	particles.second = new sf::VertexArray(sf::PrimitiveType::Points, size);
	for (size_t i = 0; i < size; i++)
	{
		particles.first.push_back(new ParticleProperties({ R::nextFloat(-100, 100), R::nextFloat(-100, 100) }, { R::nextFloat(-50, 50), R::nextFloat(-50, 50) }));
		(*particles.second)[i].position = pos;
		(*particles.second)[i].color = sf::Color(R::nextInt(200, 255), R::nextInt(100, 255), R::nextInt(0, 60));
	}
}
